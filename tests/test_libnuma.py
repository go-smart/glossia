...
def test_get_needle_parameter  ( monkeypatch , libnuma ) : 
    ...
    result = libnuma.get_needle_parameter ( random_needle_index , random_key , random_try_json )
...
def test_get_parameter     ( monkeypatch , libnuma ) : 
    ...
    monkeypatch.setattr( 'gssa.parameters.convert_parameter'  , lambda p1 , p2 , p3  : 123456 )
    result = libnuma.get_parameter ( random_key , random_try_json , random_parameters )
    assert ( result == 'key1' )
...
def test_to_xml  ( monkeypatch , libnuma ) :     
    ...
    libnuma._algorithms   = { 'first_definition' : {'arguments' : argument1 , 'content' : content1 } }
    regions1              = { "groups" : "segmented-lesions" }          
    root1                 = lxml.etree.Element('test')
    libnuma._needles      = { 'first_needle' : { 'class'  : 'point-sources'  , 'file' : file1 } }
...
{{ p.SOURCES }}
  ...
  <lesion field="DEAD"/>
    ...
    assert ( lxml.etree.tostring(result , pretty_print=True ).decode().strip() == expected_xml) 
