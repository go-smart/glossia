...
def test_get_needle_parameter  ( monkeypatch , legacy ) : 
    ...
    result = legacy.get_needle_parameter ( random_needle_index , random_key , random_try_json )
...
def test_get_parameter     ( monkeypatch , legacy ) : 
    ...
    result = legacy.get_parameter ( random_key , random_try_json , random_parameters )
    assert ( result == 123456 )
...
def test_to_xml  ( monkeypatch , legacy ) :     
    ...
    legacy._algorithms   = { 'first_definition' : {'arguments' : argument1 , 'content' : content1 } }
    regions1             = { "groups" : "segmented-lesions" }          
    root1                = lxml.etree.Element('test')
    legacy._needles      = { 'first_needle' : { 'class'  : 'point-sources'  , 'file' : file1 } }
