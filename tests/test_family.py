...
def test_get_needle_parameter ( monkeypatch , family ) :
    ...
    result = family.get_needle_parameter ( random_needle_index , random_key , random_try_json )
    assert (result == 666 )
...
def test_get_parameter ( monkeypatch , family ) :    
    ...
    monkeypatch.setattr( 'gssa.parameters.convert_parameter' , lambda p1 , p2 , p3 : True )
    result = family.get_parameter ( random_key , random_try_json , random_parameters )
    assert ( result == True ) 
